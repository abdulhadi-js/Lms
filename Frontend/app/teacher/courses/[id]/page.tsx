...
export default function CourseDetail() {
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-4 md:px-[32px] py-8 pb-24 space-y-6">
      {/* Header Section */}
      <div className="bg-surface rounded-xl border border-divider brand-shadow p-6 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
      ...
      <div className="flex space-x-1 bg-surface-container-low p-1 rounded-lg w-fit border border-divider">
        {['MODULES', 'STUDENTS', 'ASSIGNMENTS'].map(tab => (
          <button
            ...
            className={`px-6 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === tab ? 'bg-surface shadow-sm text-primary' : 'text-body-secondary hover:text-primary'}`}
          >
      ...
      {/* Tab Content */}
      <div className="bg-surface rounded-xl border border-divider brand-shadow min-h-[400px]">
              ...
              <div className="space-y-3">
                {modules.map((mod, i) => (
                  <div key={mod.id || i} className="group border border-border-light rounded-lg overflow-hidden transition-colors hover:border-primary/40">
                    <div className="p-4 bg-surface flex justify-between items-center cursor-pointer">
