...
export class MatrixGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const modulePerm = role.matrix.find(mp => mp.moduleId === requiredPermission.moduleId);

    if (!modulePerm) {
      throw new ForbiddenException(`No permissions for module ${requiredPermission.moduleId}`);
    }

    const hasAccess = (() => {
      switch (requiredPermission.action) {
        case 'VIEW': return modulePerm.canView;
        case 'ADD': return modulePerm.canAdd;
        case 'EDIT': return modulePerm.canEdit;
    ...
    if (!hasAccess) {
      throw new ForbiddenException(`Missing ${requiredPermission.action} permission for ${requiredPermission.moduleId}`);
    }
